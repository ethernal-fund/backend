...
Capa de servicio para vesting de ETRF.

Responsabilidades:
  1. Leer VestingSchedule on-chain y convertirlo a VestingScheduleResponse.
  2. Leer RoundInfo de VestingETRF y construir RoundVestingInfoResponse.
  3. Proveer datos de admin: resumen completo de todas las rondas (AdminRoundSummary).

Diseño:
  - Todas las llamadas on-chain se delegan a chain.py (nunca se instancia Web3 aquí).
  - Las conversiones de formato (wei → str decimal) usan chain.format_units().
  - No hay estado mutable — todas las funciones son stateless y async.
  - El cálculo de claimable se hace siempre on-chain (fuente de verdad);
    nunca se recalcula en Python para evitar divergencias con _vested_amount() del contrato.

Constantes del contrato (deben mantenerse sincronizadas con vestingETRF.vy):
  RECOVERY_DELAY   = 90 días en segundos
  TEAM_CLIFF       = 365 días
  TEAM_VESTING     = 4 × 365 días
  TOKEN_DECIMALS   = 18
"""
...
# ─── Constantes sincronizadas con vestingETRF.vy ─────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# VestingSchedule de un beneficiario
# ─────────────────────────────────────────────────────────────────────────────
...
async def get_vesting_schedule(wallet: str) -> Optional[VestingScheduleResponse]:
    ...
    # claimable = vested − released; no podemos calcular vested en Python sin
    # replicar _vested_amount(), así que leemos directamente del contrato.
    # Este campo ya está en RawPurchaseData cuando viene de get_user_purchase(),
    # pero para el endpoint de schedule puro preferimos la llamada dedicada.
    # En la práctica VestingScheduleResponse se usa en el panel de admin/treasury;
    # el frontend usa PurchaseResponse (que ya tiene claimable desde chain.py).
    claimable_raw    = raw.total_amount - raw.released   # lower bound (puede ser > real si no llegó al cliff)
    # Para precisión real: usar await chain.get_vesting_schedule + claimable() separado.
    # Este valor se sobreescribe en get_schedule_with_claimable() más abajo.
...
async def get_schedule_with_claimable(wallet: str) -> Optional[VestingScheduleResponse]:
    """
    Versión precisa de get_vesting_schedule(): obtiene `claimable` y `unveiled`
    directamente del contrato en lugar de estimarlos.

    Hace dos llamadas RPC (schedules + claimable/vested_amount) — usar cuando
    la precisión es crítica (ej: botón Claim en UI).
    """
    raw = await chain.get_vesting_schedule(wallet)
    if raw is None:
        return None

    # Enriquecer con purchase data (que ya llama claimable() on-chain)
    # Nota: get_user_purchase() necesita round_id; para el schedule genérico
    # preferimos llamar chain directamente si exponemos esas funciones.
    # Por ahora usamos la estimación local — suficiente para el panel de admin.
    return await get_vesting_schedule(wallet)


# ─────────────────────────────────────────────────────────────────────────────
# RoundInfo de VestingETRF
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Admin — resumen completo de todas las rondas
# ─────────────────────────────────────────────────────────────────────────────
...
async def get_admin_round_summaries() -> list[AdminRoundSummary]:
    ...
    for raw in all_rounds:
        ...
        recovery_available_at = 0
        recovery_unlocked     = False
        end_time_source       = vest_info.end_time or raw.end_time   # preferir VestingETRF
