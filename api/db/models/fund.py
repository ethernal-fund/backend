from sqlalchemy import Column, String, Boolean, DateTime, Integer, BigInteger, Numeric, ForeignKey
...
class PersonalFund(Base):
    ...
    owner_wallet     = Column(
        String(42), ForeignKey("users.wallet_address"), nullable=False, index=True
    )

    principal       = Column(Numeric(20, 6), nullable=False)   # USDC with 6 decimals
    ...
    selected_protocol = Column(
        String(42), ForeignKey("defi_protocols.protocol_address"), nullable=True
    )
    ...
    retirement_started_at     = Column(DateTime(timezone=True), nullable=True)
    ...
    next_auto_withdrawal_at          = Column(DateTime(timezone=True), nullable=True)
